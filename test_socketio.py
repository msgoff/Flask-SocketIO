...
socketio.on_event(
    "yet another custom namespace event", on_custom_event_test2, namespace="/test"
)
...
class TestSocketIO(unittest.TestCase):
    ...
    def test_broadcast_namespace(self):
        ...
        client1.emit(
            "my custom broadcast namespace event", {"a": "b"}, namespace="/test"
        )
    ...
    def test_session(self):
        ...
        client.send("echo this message back")
        self.assertEqual(
            socketio.server.environ[client.sid]["saved_session"], {"foo": "bar"}
        )
        client.send("test session")
        self.assertEqual(
            socketio.server.environ[client.sid]["saved_session"],
            {"a": "b", "foo": "bar"},
        )
